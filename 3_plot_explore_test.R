...
saved_simulations <- append_epi_outputs_to_df(saved_simulations)
...
#this_fit_adj <- 0.95
subset_for_plotting <- adjust_fitness(subset_for_plotting, fit_adj_new = this_fit_adj)
subset_for_plotting <- add_competition_and_invasion_columns(subset_for_plotting)

if (ttype == 1 & mainvecpop ==F & this_fit_adj == 0.8) {df_ttype1_F <- subset_for_plotting}
if (ttype == 2 & mainvecpop ==F & this_fit_adj == 0.8) {df_ttype2_F <- subset_for_plotting}
if (ttype == 3 & mainvecpop ==F & this_fit_adj == 0.8) {df_ttype3_F <- subset_for_plotting}
if (ttype == 1 & mainvecpop ==T & this_fit_adj == 0.8) {df_ttype1_T <- subset_for_plotting}
if (ttype == 2 & mainvecpop ==T & this_fit_adj == 0.8) {df_ttype2_T <- subset_for_plotting}
if (ttype == 3 & mainvecpop ==T & this_fit_adj == 0.8) {df_ttype3_T <- subset_for_plotting}
...
# Generate plots ---------------------------------------------------------------
common_details <- paste0("_ttype", ttype, "_spec_", spec, "_vector_value_", this_vector_measure_value)
common_details_with_fitness <- paste0("_ttype", ttype, "_spec_", spec, "_vector_value_", this_vector_measure_value, "_fit_adj_", this_fit_adj)
...
plot_label <- paste0(plot_label, "_ttype", ttype, "_spec_", spec)
plots[[plot_label]] <- p
my_ggsave(plot = p,
...
plot_label <- paste0("plot_type0_Rres_Rsen_ratio", common_details_with_fitness)
output_filename <- paste0(folder_name, plot_label, ".pdf")
plots[[plot_label]] <- p
 my_ggsave(plot = p,
   filename = output_filename,
...
for (y_var in y_vars) {
  ...
  plot_label <- paste0("plot_type1_", y_var, common_details)
  output_filename <- paste0(folder_name, plot_label, ".pdf")
  plots[[plot_label]] <- p
  my_ggsave(plot = p,
    filename = output_filename,
...
p <- plot_invasion_landscape(prev_threshold, restricted_subset, ttype, mainvecpop)
...
with_contours = TRUE
p <- plot_invasion_landscape(prev_threshold, restricted_subset, ttype, mainvecpop, panel_type = "single", with_contours)
...
y_vars <- c("RiskA")
for (y_var in y_vars) {
p <- plot_other_landscape(restricted_subset, y_var, max_value = 13.0, ttype)
...
y_vars <- c("RiskA")
print(" ")
print(paste0(ttype, mainvecpop))
print(" ")
for (y_var in y_vars) {
  p <- plot_other_landscape(restricted_subset, y_var, max_value = 13.0, ttype, panel_type = "single")
...
p <- plot_type8(subset_for_plotting, ttype)
