...
p1 <- plot_boundary_tidy_Number_treated(plot_this_filtered, "treat_prop", treat_prop_thresh) 
p1 <- p1 + ggtitle(paste0("Maximum proportion\nof cases treated: ", treat_prop_thresh))

treat_prop_thresh <- 0.5
p2 <- plot_boundary_tidy_Number_treated(plot_this_filtered, "treat_prop", treat_prop_thresh)
...
unique(plot_this1$treatments_per_year)
desired_treatments_per_year <- round(c(seq(0, 0.9, by = 0.1),seq(1, 3.8, by = 0.2), seq(4, 9, by = 0.5)), 3)
desired_treatments_per_year <- round(unique(plot_this1$treatments_per_year), 3)
plot_this_filtered <- plot_this1 %>% 
  filter(treatment_code == 3, 
         NW == 100, prop_cattle_with_insecticide <= 0.8, Insecticide_strategy == this_insecticide_strategy,
