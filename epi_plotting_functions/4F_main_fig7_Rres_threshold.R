...
p1 <- plot_boundary_tidy_Rres(plot_this_filtered, "treat_prop", treat_prop_thresh) 
p1 <- p1 + ggtitle(paste0("Maximum proportion\nof cases treated: ", treat_prop_thresh))

treat_prop_thresh <- 0.5
p2 <- plot_boundary_tidy_Rres(plot_this_filtered, "treat_prop", treat_prop_thresh)
p2 <- p2 + ggtitle(paste0("Maximum proportion\nof cases treated: ", treat_prop_thresh))

treat_prop_thresh <- 0.25
p3 <- plot_boundary_tidy_Rres(plot_this_filtered, "treat_prop", treat_prop_thresh)
p3 <- p3 + ggtitle(paste0("Maximum proportion\nof cases treated: ", treat_prop_thresh))

pA <- p1 + p2 +p3 + plot_layout(guides = 'collect', axes = 'collect') 
pA
filename = paste0("4F_main_fig7A_responsive_", "Insecticide_strategy_", 
...
treat_freq_thresh <- 6
p1 <- plot_boundary_tidy_Rres(plot_this_filtered, "treatments_per_year", treat_freq_thresh) 
p1 <- p1 + ggtitle(paste0("Max herd treatment\nfrequency ", treat_freq_thresh, " per year"))


treat_freq_thresh <- 3
p2 <- plot_boundary_tidy_Rres(plot_this_filtered, "treatments_per_year", treat_freq_thresh)
p2 <- p2 + ggtitle(paste0("Max herd treatment\nfrequency ", treat_freq_thresh, " per year"))


treat_freq_thresh <- 1
p3 <- plot_boundary_tidy_Rres(plot_this_filtered, "treatments_per_year", treat_freq_thresh)
