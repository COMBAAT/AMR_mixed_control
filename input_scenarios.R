...
create_multiple_scenarios <- function() {
  ...
  prop.insecticide.vec <- c(0.0, 0.05, 0.10, 0.15, 0.2, 0.5)
  ...
  df <- expand.grid(
    emergence.adj = emergence.adj.vec, dose.adj = dose.adj.vec,
    treat_prop = treat.prop.vec, NW = N_wl, K = K.vec,
    fit.adj = fit.adj.vec, prop.insecticide = prop.insecticide.vec,
    birth.adj = birth.adj.vec, prop.prophylaxis = prop.prophylaxis,
