...
create_multiple_scenarios <- function() {
  ...
  treat_prop_vecA <- seq(0, 0.9, by = 0.2) # full from 0-1
  treat_prop_vecB <- seq(0.91, 0.99, by = 0.02)
  treat_prop_vec <- c(treat_prop_vecA, treat_prop_vecB)
  ...
  dose_adj_vec <- 1 # c(1, 0.9, 0.7, 0.4, 0.2)
  emergence_adj_vec <- 1

  df <- expand.grid(
    emergence_adj = emergence_adj_vec, dose_adj = dose_adj_vec,
    treat_prop = treat_prop_vec, NW = N_wl, K = K.vec,
    fit_adj = fit_adj_vec, prop_insecticide = prop_insecticide.vec,
    birth_adj = birth_adj_vec, prop_prophylaxis = prop_prophylaxis,
