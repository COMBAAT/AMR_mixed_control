...
create_single_scenario_new <- function() {
  ...
  cattle_number <- 100
  wildlife_number <- 0
  hosts <- cattle_number + wildlife_number
  ...
  treatment_code <- 1 # 1, 2, 3
  if (treatment_code == 1) {
    treatment_type <- "curative"
    laXbel <- "responsive_curative"
    treat_prop <- 0.1
    treatments_per_year <- 0
    proph_ongoing <- treatments_per_year / days_per_year
  } else if (treatment_code == 2) {
    treatment_type <- "longlasting"
    laXbel <- "responsive_longlasting"
    treatments_per_year <- 0
    proph_ongoing <- treatments_per_year / days_per_year
    treat_prop <- 0.1
  } else if (treatment_code == 3) {
    treatment_type <- "longlasting"
    laXbel <- "proph_ongoing"
    treatments_per_year <- 1
    treat_prop <- 0.0
    proph_ongoing <- treatments_per_year / days_per_year
  }
  ...
  tb0 <- expand.grid(
    ...
    maintain_vector_pop = maintain_vector_pop,
    fit_adj = fit_adj, 
    prop_cattle_with_insecticide = prop_cattle_with_insecticide,
    birth_adj = birth_adj, 
    prop_prophylaxis_at_birth = prop_prophylaxis_at_birth,
    max_time = max_time,
    treatment_type = treatment_type,
    treat_prop = treat_prop,
    treatments_per_year = treatments_per_year,
    laXbel = laXbel,
    treatment_code = treatment_code,
    proph_ongoing = proph_ongoing
  )
  df <- tb0 %>% mutate(use_carrying_capacity = FALSE, K = K_host_ratio * hosts)
