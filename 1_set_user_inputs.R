...
get_user_inputs <- function() {
  user_inputs <- list(
    ...
    current_descriptor = "NovemberA_proph"
  )
  user_inputs
}
...
create_multiple_scenarios <- function() {
  ...
  carrying_capacity <- c(10000, 6000, 4000, 1000, 500)
  ...
  proph_ongoing <- seq(0, 1, 2) / days_per_year
  fit_adj <- 0.8
  birth_adj <- 2.0
  dose_adj <- 1.0
  emergence <- 0.0
...
create_single_scenario <- function() {
  days_per_year <- set_days_per_year()
  max_time <- 5000
  treatment_type <- sample(c("quick", "proph"), 1) # quick or proph
  cattle_number <- 100
  wildlife_number <- 50
  treat_prop <- 0.09
